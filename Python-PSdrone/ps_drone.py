...
######################=-
	...
	def getSelfRotation(self, wait):
		...
		reftime = time.time()
		oangle = self.__NavData["f"][2][2]				# detects the self-rotation-speed of the yaw-sensor
		time.sleep(wait)
						...
						if self.__ConfigData[i][0].find(self.__ConfigQueue[0][0]) > -1:
							if self.__ConfigData[i][1] != self.__ConfigQueue[0][1]:
								if self.debug:
									...
									print("   "+self.__ConfigData[i][0]+" should be \""+self.__ConfigQueue[0][1]+"\" is \""+self.__ConfigData[i][1]+"\""+self.__ConfigQueue.append(self.__ConfigQueue[0]))		# If value is not correctly set, requeue !
...
def reconnect(navdata_pipe, commitsuicideND, DroneIP,NavDataPort):
	if not commitsuicideND:		navdata_pipe.sendto("\x01\x00\x00\x00".encode(), (DroneIP, NavDataPort))
